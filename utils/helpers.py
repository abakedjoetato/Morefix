...
async def ensure_awaited(possible_coroutine):
    """
    Ensures that a possible coroutine is properly awaited.
    
    Args:
        possible_coroutine: Any value that might be a coroutine
        
    Returns:
        The awaited result if it was a coroutine, otherwise the original value
    """
    if inspect.iscoroutine(possible_coroutine):
        return await possible_coroutine
    return possible_coroutine

async def safe_embed_send(ctx, embed_or_coroutine, **kwargs):
    """
    Safely sends an embed, ensuring it's properly awaited if needed.
    
    Args:
        ctx: Command context or interaction
        embed_or_coroutine: Discord Embed or coroutine that returns one
        **kwargs: Additional arguments for send
        
    Returns:
        The sent message
    """
    # Ensure embed is ready to use
    embed = await ensure_awaited(embed_or_coroutine)
...
async def throttle(coro, max_calls: int, interval: float, key: Optional[Callable] = None):
    ...
    """
    # Use default key function if none is provided
    if key is None:
        key = lambda: "default"
    ...
    if isinstance(state, dict) and state["calls"] >= max_calls:
