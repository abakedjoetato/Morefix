...
async def get_minimum_tier_for_feature(feature_name: str) -> int:
    ...
    """
    # This should not be awaited as get_feature_tier_requirement is not async
    # Convert string tier to numeric tier
    tier_name = get_feature_tier_requirement(feature_name)
    tier_map = {"free": 0, "basic": 1, "pro": 2, "enterprise": 3}
    return tier_map.get(tier_name.lower(), 0) if tier_name else 0
...
async def validate_server_limit(guild) -> Tuple[bool, Optional[str]]:
    ...
    try:
        ...
        # Convert the numeric tier to string for SERVER_LIMITS dictionary lookup
        tier_key = str(guild_tier) if isinstance(guild_tier, int) else guild_tier
        server_limit = SERVER_LIMITS.get(tier_key, 1)
