...
class Events(commands.Cog):
    ...
    @app_commands.choices(event_type=[
        utils.discord_patches.app_commands.Choice(name="All Events", value="all"),
        utils.discord_patches.app_commands.Choice(name="Missions", value="mission"),
        utils.discord_patches.app_commands.Choice(name="Airdrops", value="airdrop"),
        utils.discord_patches.app_commands.Choice(name="Helicopter Crashes", value="crash"),
        utils.discord_patches.app_commands.Choice(name="Traders", value="trader"),
        utils.discord_patches.app_commands.Choice(name="Convoys", value="convoy"),
        utils.discord_patches.app_commands.Choice(name="Special Encounters", value="encounter"),
        utils.discord_patches.app_commands.Choice(name="Server Restarts", value="server_restart")
    ])
    @app_commands.autocomplete(server_id=server_id_autocomplete)
    @premium_feature_required(feature_name="events", min_tier=1)  # Events monitoring requires premium tier 1+
...
async def start_events_monitor(bot, guild_id: int, server_id: str):
    ...
    try:
        ...
        if sftp_key in bot.sftp_connections:
            ...
            if sftp_client and not sftp_client.connected:
