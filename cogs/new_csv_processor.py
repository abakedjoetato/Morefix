...
class CSVProcessorCog(commands.Cog):
    ...
    @commands.slash_command(name="process_csv", description="Manually process CSV files from the game server")
    ...
    @commands.slash_command(name="historical_parse", description="Process historical CSV data")
    @app_commands.describe(
        server_id="Server ID to process (optional)",
        days="Number of days to look back (default: 30)"
    )
    ...
    @commands.slash_command(name="csv_status", description="Show CSV processor status")
