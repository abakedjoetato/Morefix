...
from typing import Union,  Optional, List, Dict, Any, Union, cast
import random
import asyncio
import traceback
...
from utils.premium import validate_premium_feature  # Keep for backward compatibility
...
class Economy(commands.Cog):
    ...
    @premium_feature_required(feature_name="economy", min_tier=2)  # Economy requires premium tier 2+
    ...
    async def leaderboard(self, ctx, server_id: str):
        ...
        if richest_players is None or len(richest_players) == 0:
            embed = await EmbedBuilder.create_error_embed(
                "No Data",
    ...
    @app_commands.autocomplete(server_id=server_id_autocomplete)
    @premium_feature_required(feature_name="economy", min_tier=2)  # Gambling requires premium tier 2+
    async def blackjack(self, ctx, server_id: str, bet: int = 10):
        ...
        if isinstance(game_state, dict) and game_state["game_over"]:
