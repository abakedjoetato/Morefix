...
class Killfeed(commands.Cog):
    ...
    @premium_feature_required(feature_name="killfeed", min_tier=0)  # Killfeed available in free tier (0+)
    async def start(self, ctx, server_id: str):
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"No valid killfeed servers found. Please configure servers first.")
    ...
    try:
        ...
        if "-" in server.server_id and len(server.server_id) > 30 and original_server_id == server.get("server_id"):
            ...
            if hasattr(server, 'name') and server.name:
                ...
                for word in str(server.name).split():
                    if word.isdigit() and len(word) >= 4:
                        logger.info(f"Found potential numeric server ID in server_name: {word}")
                        original_server_id = word
                        break
        
        logger.info(f"Using original_server_id: {original_server_id} for path construction in killfeed")
        ...
        bot.sftp_connections[f"{server_id}"] = sftp_client
        ...
        if channel_configured and guild:  # Only try to get the channel if guild exists
            ...
            try:
                if killfeed_channel_id is not None:
                    ...
                    logger.info(f"Attempted to get channel: {killfeed_channel_id}, result: {killfeed_channel}")
...
async def process_kill_event(bot, server, kill_event, channel):
    ...
    try:
        ...
        kill_event["server_id"] = server.get("server_id")
        ...
        if is_suicide is not None:
            ...
            if suicide_type in server.suicide_notifications and not server.suicide_notifications.get(suicide_type, True):
                logger.debug(f"Skipping notification for {suicide_type} suicide as it's disabled for server {server.get('server_id')}")
                # We still update player stats, but don't send a message
                await update_player_stats(bot, server.get('server_id'), kill_event)
                return
        ...
        embed = await EmbedBuilder.create_kill_embed(kill_event, guild=guild_model)
        ...
        if has_economy is not None:
            try:
                if not kill_event["is_suicide"]:
                    # Get killer's economy info
                    from models.economy import Economy
                    killer_id = kill_event["killer_id"]
                    killer_economy = await Economy.get_by_player(bot.db, killer_id, server.get("server_id"))
                    ...
                    if killer_economy is not None:
                        ...
                        killer = await Player.get_by_id(bot.db, killer_id, server.get("server_id"))
...
async def update_player_stats(bot, server_id, kill_event):
    ...
    try:
        ...
        # Handle suicide case
        if isinstance(kill_event, dict) and kill_event["is_suicide"]:
            ...
            if killer and victim:
                ...
                logger.warning(f"Could record kill: killer={not killer}, victim={victim}")
            ...
            if has_rivalries is not None:
                ...
                logger.warning(f"Could record death: killer={not killer}, victim={victim}")
            ...
            elif has_economy is not None:
                logger.warning(f"Could award currency: killer={not killer}, victim={victim}")
