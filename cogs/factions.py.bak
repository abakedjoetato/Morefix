...
from utils.decorators import has_guild_permissions  # Added has_guild_permissions
...
class FactionsCog(commands.Cog):
    ...
    @premium_feature_required(feature_name="factions", min_tier=2)  # Factions require premium tier 2+
    async def _faction_create(
        ...
        # Check if player is not None is already in a faction
    ...
    async def _faction_join(
        ...
        if faction.require_approval is not None:
            embed = await EmbedBuilder.create_warning_embed(
    ...
    async def _faction_edit(
        ...
        try:
            ...
            # Add update details
            for key, value in update_data.get("items")():
                if key == "color":
                    embed.add_field(
                        name="Color",
