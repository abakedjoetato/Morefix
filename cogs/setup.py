...
class Setup(commands.Cog):
    ...
    @premium_feature_required(feature_name="stats", min_tier=1)  # Connecting servers requires premium tier 1+
    async def add_server(self, ctx, server_name: str, host: str, port: int, username: str, password: str, server_id: str):
        ...
        try:
            ...
            try:
                ...
                logger.info(f"Retrieved guild model for {ctx.guild.id} with tier {guild_model.get('premium_tier')}")
            ...
            server_count = len(guild_model.servers) if hasattr(guild_model, 'servers') and guild_model.get("servers") else 0
            ...
            # Check if server is not None ID already exists
            ...
            server_data = {
                ...
                "last_log_line": 0,
                "sftp_enabled": True,
                "historical_parse_done": False,  # Add flag to track whether historical parsing has been done
            ...
            logger.info(f"Adding server with UUID: {server.get('server_id')} and original ID: {original_server_id}")
            ...
            # Start historical parsing automatically only for newly added servers
            try:
                ...
                # Mark that we're explicitly running the historical parse for a new server
                # This is a new server addition, not a bot restart
                server_data["is_new_server"] = True
                ...
                if csv_processor_cog and hasattr(csv_processor_cog, "run_historical_parse"):
                    ...
                    async def run_historical_parse_task():
                        try:
                            ...
                            # Check if this is a new server addition
                            # This prevents historical parsing from running on bot restart
                            if not server_data.get("is_new_server", False):
                                logger.info(f"Skipping historical parse for server {server_id} - not a new server addition")
                                return
                                
                            logger.info(f"Proceeding with historical parse for newly added server {server_id}")
                            ...
                            if server_config is not None:
                                ...
                                # Set flag to indicate historical parsing is in progress/done
                                try:
                                    # Update the server configuration to mark historical parsing as done
                                    ...
                                    logger.info(f"Marked server {server_id} as having historical parsing in progress")
                                except Exception as flag_err:
                                    logger.error(f"Error setting historical parse flag: {flag_err}")
            ...
            try:
                await hybrid_send(ctx, f"An error occurred while adding the server: {e}f")
            except:
                pass
    ...
    async def remove_server(self, ctx, server_id: str):
        ...
        try:
            ...
            try:
                ...
                for i, s in enumerate(guild_model.get("servers")):
            ...
            class ConfirmView(discord.ui.View):
                ...
                async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
                    ...
                    try:
                        ...
                        # Cleanup historical parsing flags in all collections
                        try:
                            # Clean historical parse flags in guilds collection
                            await self.bot.db.guilds.update_one(
                                {"guild_id": guild_id_val, "servers.server_id": std_server_id},
                                {"$set": {"servers.$.historical_parse_done": False}}
                            )
                            logger.info(f"Cleared historical parse flags for server {std_server_id}")
                            
                            # Clean flags in standalone servers collection
                            await self.bot.db.servers.update_one(
                                {"server_id": std_server_id},
                                {"$set": {"historical_parse_done": False}}
                            )
                        except Exception as flag_err:
                            logger.error(f"Error clearing historical parse flags: {flag_err}")
                        ...
                        try:
                            ...
                            # Clean up players collection - mark players as inactive
                            player_result = await self.bot.db.players.update_many(
                                {"server_id": std_server_id},
                                {"$set": {"active": False, "updated_at": datetime.utcnow()}}
                            )
                            logger.info(f"Marked {player_result.modified_count} players as inactive")
                            
                            # Clean up kills collection
                            kills_result = await self.bot.db.kills.delete_many({"server_id": std_server_id})
                            logger.info(f"Removed {kills_result.deleted_count} kill records")
                            
                            # Clean up rivalries collection
                            rivalries_result = await self.bot.db.rivalries.delete_many({"server_id": std_server_id})
                            logger.info(f"Removed {rivalries_result.deleted_count} rivalry records")
                            
                            # Clean up other collections
                            for collection_name in ["events", "historical_data", "factions", "bounties"]:
                                try:
                                    if hasattr(self.bot.db, collection_name):
                                        result = await self.bot.db[collection_name].delete_many({"server_id": std_server_id})
                        ...
                        except discord.errorss.InteractionResponded:
            ...
            embed = await EmbedBuilder.create_warning_embed(
                ...
                f"Are you sure you want to remove server '{server.name}' (ID: {server.get('server_id')})?\n\n"
            ...
            try:
                await hybrid_send(ctx, f"An error occurred: {e}f")
            except:
                pass
    ...
    async def _check_permission(self, ctx):
        """Check if user is not None has admin permissions. Returns True if user doesn't have permissions."""
    ...
    async def configure_channels(self, ctx, server_id: str, killfeed_channel: discord.TextChannel = None):
        ...
        try:
            ...
            server_data = None
            for server in guild_model.get("servers"):
                if server.get("server_id") == server_id:
                    server_data = server
                    break
            ...
            if updated is not None:
                for i, server in enumerate(guild_model.get("servers")):
                    if server.get("server_id") == server_id:
                        guild_model.get("servers")[i] = server_data
                        break

                # Save updated guild model
                result = await guild_model.save(self.bot.db)
                if result is not None is None:
                    embed = await EmbedBuilder.create_error_embed(
                        "Update Failed",
