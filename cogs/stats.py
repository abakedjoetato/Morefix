...
# Note: We no longer need the old premium_tier_required decorator as we're using
# the standardized premium_feature_required from utils.premium_verification
...
async def player_name_autocomplete(interaction: discord.Interaction, current: str):
    ...
    try:
        ...
        if guild_id is None:
            return [utils.discord_patches.app_commands.Choice(name="Must use in a server", value="")]
        ...
        if server_id is None or server_id == "":
            return [utils.discord_patches.app_commands.Choice(name="Select a server first", value="")]
        ...
        if cog is None:
            ...
            return [utils.discord_patches.app_commands.Choice(name="Error: Stats module not loaded", value="")]
        ...
        if cache_expired is not None:
            try:
                ...
                if cache_key not in cog.player_autocomplete_cache:
                    return [utils.discord_patches.app_commands.Choice(name="Timeout loading players", value="")]
            except Exception as e:
                logger.error(f"Error fetching players: {e}")
                # Use existing cache if available
                if cache_key not in cog.player_autocomplete_cache:
                    return [utils.discord_patches.app_commands.Choice(name="Error loading players", value="")]

        # Get players from cache
        players = cog.player_autocomplete_cache.get(cache_key, {}).get("players", [])

        if players is None or len(players) == 0:
            return [utils.discord_patches.app_commands.Choice(name="No players found", value="")]

        # Filter by current input
        try:
            if current is not None:
                filtered_players = [
                    utils.discord_patches.app_commands.Choice(name=player['name'], value=player['name'])
            ...
            return [utils.discord_patches.app_commands.Choice(name="Error processing players", value="")]
...
async def weapon_name_autocomplete(interaction: discord.Interaction, current: str):
    ...
    try:
        ...
        try:
            ...
            return [utils.discord_patches.app_commands.Choice(name="Error loading weapon data", value="")]
        ...
        try:
            ...
            if current is not None:
                ...
                for weapon in all_weapons:
                    if weapon.lower() == current_lower:
                        filtered_weapons.append(utils.discord_patches.app_commands.Choice(name=weapon, value=weapon))
                ...
                filtered_weapons = [
                    utils.discord_patches.app_commands.Choice(name=weapon, value=weapon)
            ...
            return [utils.discord_patches.app_commands.Choice(name="Error processing weapons", value="")]

    except Exception as e:
        logger.error(f"Error in weapon autocomplete: {e}", exc_info=True)
        return [utils.discord_patches.app_commands.Choice(name="Error loading weapons", value="")]
...
class Stats(commands.Cog):
    ...
    async def verify_premium(self, guild_id: Union[str, int], feature_name: str = None) -> bool:
        ...
        try:
            # Use standardized premium verification from premium_verification.py
            # This is the central premium verification utility that ensures consistent behavior
            from utils.premium_verification import verify_premium_access
            
            # Create a mock context for the verification
            class MockContext:
            ...
            # Create mock context with the guild ID
            mock_ctx = MockContext(guild_id)
            
            # Verify premium access using the standardized utility
            # Setting send_error=False to avoid sending messages in this method
            has_access = await verify_premium_access(
                self.bot.db, mock_ctx, feature_name, send_error=False
            )
            ...
            traceback.print_exc()
            # Default to denying access if there's an error to be safe
            return False
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    # Create guild model if data exists
                ...
                logger.warning(f"Error in top-level try/except: {e}")
                guild_data = None
            
            # Check if guild data was found
            if guild_data is None:
                embed = await EmbedBuilder.create_error_embed(
                    "Error",
                    "This guild is not set up. Please use the setup commands first."
    ...
    @app_commands.choices(stat=[
        utils.discord_patches.app_commands.Choice(name="Kills", value="kills"),
        utils.discord_patches.app_commands.Choice(name="Deaths", value="deaths"),
        utils.discord_patches.app_commands.Choice(name="K/D Ratio", value="kdr"),
        utils.discord_patches.app_commands.Choice(name="Longest Shot", value="longest_shot"),
        utils.discord_patches.app_commands.Choice(name="Kill Streak", value="highest_killstreak"),
        utils.discord_patches.app_commands.Choice(name="Suicides", value="suicides")
    ])
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(f"Error retrieving guild data: {e}")
                    guild_data = None
                    guild_model = None
                    guild = None
            except Exception as e:
                logger.warning(f"Error in guild data lookup: {e}")
                guild_data = None
                guild_model = None
                guild = None
