    ...
    def decorator(func):
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            try:
                ...
                logger.error(f"Error during operation {operation_id} for server {self.server_id}: {e}f")
                raise
            finally:
...
async def get_sftp_client(
    ...
    **kwargs  # For additional parameters
    ...
    Args:
        ...
        original_server_id: Original numeric server ID for path construction
    ...
    # Check if SFTP is enabled in the environment
    if not SFTP_ENABLED:
        logger.warning("SFTP client requested while SFTP is disabled. Set SFTP_ENABLED=true in .env to enable.")
        # Create a client that will report as disconnected
        client = SFTPClient(
            hostname=hostname or sftp_host or kwargs.get('host') or "localhost",
            port=port or sftp_port or kwargs.get('port') or 22,
            username=username or sftp_username or kwargs.get('user') or "disabled",
            password=password or sftp_password or kwargs.get('pwd') or "disabled",
            server_id=server_id or f"disabled_{random.randint(1000, 9999)}"
        )
        client._connected = False
        client.last_error = "SFTP functionality is disabled"
        return client
...
class SFTPManager:
    ...
    def __init__(
        ...
        try:
            ...
            # Start with reasonable default: provided original_server_id or fallback to server_id
            ...
            # This handles the case where we're given a UUID as original_server_id
                ...
                if original_server_id in KNOWN_SERVERS:
                    ...
                    numeric_id, is_known = identify_server(
                        server_id=original_server_id if original_server_id is not None else server_id,
            ...
            # 5. Priority 5: No direct numeric ID found, use server_identity to find one
                ...
                numeric_id, is_known = identify_server(
                    server_id=server_id if server_id is not None else "",  # Ensure we pass a string, not None
    ...
    async def connect(self) -> 'SFTPManager':
        ...
        # Check if SFTP is enabled in the environment
        if not SFTP_ENABLED:
            logger.warning("SFTP connection attempt made while SFTP is disabled. Set SFTP_ENABLED=true in .env to enable.")
            self.last_error = "SFTP functionality is disabled"
            self.client = None
            return self
    ...
    async def get_file_attrs(self, path: str) -> Optional[Any]:
        ...
        try:
            ...
            if file_info and isinstance(file_info, dict):
                ...
                # Check if file_info contains permissions
                if 'permissions' in file_info:
                    return FileAttrs(file_info['permissions'])
                # Check if it's a directory based on is_dir flag
                elif file_info.get('is_dir', False):
                    return FileAttrs(stat.S_IFDIR | 0o755)  # Directory with standard permissions
                elif file_info.get('is_file', False):
                    return FileAttrs(stat.S_IFREG | 0o644)  # File with standard permissions
                    
            # If all methods fail, return None
            return None
        except Exception as e:
            logger.error(f"Failed to get file attributes for {path}: {e}")
            return None
    ...
    async def list_files(self, directory: str, pattern: Optional[str] = None) -> List[str]:
        ...
        Returns:
            List[str]: List of filenames (relative to the directory, NOT full paths)
        """
        if not self.client:
            logger.error(f"SFTP client is missing when trying to list files: {directory}")
            return []

        # CRITICAL FIX: Sanitize directory path
        # Ensure there are no doubled slashes that could cause path issues
        if directory:
            directory = directory.replace('//', '/')

        try:
            # Log the exact pattern being used for file filtering
            if pattern:
                logger.info(f"Listing files in {directory} with pattern: {pattern}")

            if hasattr(self.client, 'list_files'):
                # Using client's dedicated list_files method
                files = await self.client.list_files(directory, pattern or '')
                logger.info(f"Found {len(files)} files using client.list_files in {directory}")
                return files
                ...
                if self.client and self.client._sftp_client:
                    # Using raw SFTP listdir method
                    files = await self.client._sftp_client.listdir(directory)
                    logger.info(f"Found {len(files)} files using _sftp_client.listdir in {directory}")
                    return files
                return []
            else:
                # Fallback to find_files_by_pattern
                if hasattr(self.client, 'find_files_by_pattern'):
                    # Use find_files pattern matching for more flexibility
                    files = await self.client.find_files_by_pattern(directory, pattern or '.*', recursive=False)
                    logger.info(f"Found {len(files)} files using find_files_by_pattern in {directory}")
                    
                    # CRITICAL FIX: Ensure we're returning just filenames, not full paths
                    # If the client returned full paths, extract just the filename part
                    if files and files[0].startswith('/'):
                        # Strip the directory prefix to get just the filenames
    ...
    async def read_file(self, remote_path: str, start_line: int = 0, max_lines: int = -1) -> Optional[Union[bytes, List[str]]]:
        ...
        Returns:
            Optional[Union[bytes, List[str]]]: File content as bytes or list of lines
        """
        if not self.client:
            logger.error(f"SFTP client is missing when trying to read file: {remote_path}")
            return None

        # CRITICAL FIX: Sanitize remote path
        # Ensure there are no doubled slashes that could cause path issues
        if remote_path:
            remote_path = remote_path.replace('//', '/')
            logger.debug(f"Sanitized path for reading: {remote_path}")

        try:
            # Log that we're attempting to read from this file
            logger.info(f"Reading file: {remote_path}")
            
            # First try to directly read the file content if the client supports it
            if hasattr(self.client, 'read_file'):
                content = await self.client.read_file(remote_path, start_line, max_lines)
                
                # CRITICAL FIX: Log the content type for better debugging
                if content is not None:
                    logger.info(f"Successfully read content from {remote_path} " 
                              f"(type: {type(content)}, length: {len(content) if hasattr(content, '__len__') else 'N/A'})")
                    return content
                else:
                    logger.warning(f"No content read from {remote_path}")
                    return None
            else:
                # Fallback to download_file if read_file is not available
                logger.info(f"Falling back to download_file for {remote_path}")
                content = await self.download_file(remote_path)
                if isinstance(content, bytes):
                    # If we receive bytes, return it directly
                    logger.info(f"Successfully downloaded {len(content)} bytes from {remote_path}")
                    return content
                else:
                    logger.warning(f"Failed to download content from {remote_path}")
                    return None
        except Exception as e:
        ...
        try:
            if self.client:
                ...
                try:
                    ...
                    logger.warning(f"Error during client disconnect: {e}f")

                # Additional cleanup
                logger.debug("SFTP disconnect completed successfully")
        except Exception as e:
            logger.error(f"Error in disconnect cleanup: {e}f")
            ...
            try:
                ...
                for path in paths_to_try:
                    ...
                    try:
                        ...
                        logger.debug(f"Log file not found at {path}: {e}f")
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                if recursive:
                    ...
                    logger.debug(f"Using recursive search for CSV files in {directory} (max_depth={max_depth})")
                ...
                if len(files) > 0:
                    logger.info(f"Total CSV files found: {len(files)}")
                return files
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                try:
                    ...
                    # If listing fails, try to get file attributes
                    try:
                        attrs = await self.client.get_file_attrs(path)
        ...
        try:
            ...
            try:
                ...
                if isinstance(self.client, asyncssh.SFTPClient):
                    ...
                    try:
                        ...
                        logger.debug(f"Downloaded file using AsyncSSH readfile ({len(content)} bytes)")
                        return content
                    except Exception as ssh_err:
                        logger.debug(f"AsyncSSH readfile failed: {ssh_err}, trying other methods")
            except (ImportError, Exception) as e:
            ...
            if hasattr(self.client, 'getfo'):
                try:
                    ...
                    logger.debug(f"Downloaded file using getfo ({len(content)} bytes)")
                    return content
            ...
            if hasattr(self.client, 'read'):
                try:
                    ...
                    logger.debug(f"Downloaded file using read ({len(content)} bytes)")
                    return content
            ...
            if hasattr(self.client, 'open'):
                try:
                    ...
                    logger.debug(f"Downloaded file using open+read ({len(content)} bytes)")
                    return content
                ...
                if 'asyncssh' in client_type or 'sftp' in client_type:
                    ...
                    for method_name in ['readfile', 'getfile', 'download', 'read_file', 'fetch']:
                        if hasattr(self.client, method_name):
                            try:
                                logger.debug(f"Trying alternative method: {method_name}")
                                ...
                                if method_name in ['readfile', 'read_file', 'fetch', 'download']:
                                    ...
                                    if content:
                                        if isinstance(content, str):
                                            content = content.encode('utf-8') 
                                        logger.debug(f"Downloaded file using {method_name} ({len(content)} bytes)")
                                        return content
                                else:
...
class SFTPClient:
    ...
    async def connect(self) -> 'SFTPClient':
        ...
        try:
            ...
            self._connected = False
            self.last_error = f"{type(e).__name__}: {e}f"
    ...
    async def ensure_connected(self):
        ...
        try:
            ...
            self.last_error = f"Connection refused: {e}f"
            ...
            self.last_error = f"Connection error: {e}f"
    ...
    async def get_file_attrs(self, path: str) -> Optional[Any]:
        ...
        try:
            if self._sftp_client:
                ...
                logger.error(f"SFTP client is missing when trying to get file attributes for {path}")
                return None
        except Exception as e:
            logger.error(f"Failed to get file attributes for {path}: {e}")
            return None
    ...
    async def _download_to_memory(self, remote_path: str) -> Optional[bytes]:
        ...
        try:
            ...
            try:
                ...
                if isinstance(self._sftp_client, asyncssh.SFTPClient):
                    ...
                    try:
                        ...
                        async with await self._sftp_client.open(remote_path, 'rb') as f:
                            content = await f.read()
                        self.last_activity = datetime.now()
                        if isinstance(content, str):
                            content = content.encode('utf-8')
            ...
            try:
                ...
                if 'asyncssh' in client_type or 'sftp' in client_type:
                    # Try some common methods that might be available - AVOID read_file which is our own method
                    for method_name in ['readfile', 'getfile', 'download', 'fetch']:
                        if hasattr(self._sftp_client, method_name):
                            try:
                                logger.info(f"Trying alternative method: {method_name}")
                                
                                if method_name in ['readfile', 'fetch', 'download']:
                                    # Methods that return content directly
            ...
            # Method 5: Use get with a temporary file (least efficient but reliable fallback)
            # Note: AsyncSSH doesn't have 'get', it uses open+write instead
            temp_file = f"temp_{int(time.time())}.tmp"
            try:
                # For AsyncSSH compatibility - uses open+write instead of get
                try:
                    import asyncssh
                    if isinstance(self._sftp_client, asyncssh.SFTPClient):
                        logger.info(f"Using AsyncSSH open+write method instead of get for temp file")
                        async with await self._sftp_client.open(remote_path, 'rb') as remote_file:
                            content = await remote_file.read()
                            with open(temp_file, 'wb') as local_file:
                                local_file.write(content)
                    else:
                        # Use standard get method for other clients
                        await self._sftp_client.get(remote_path, temp_file)
                except ImportError:
                    # If asyncssh isn't available, try the standard get method
                    await self._sftp_client.get(remote_path, temp_file)
            ...
            if local_path:
                ...
                try:
                    ...
                    try:
                        import asyncssh
                        if isinstance(self._sftp_client, asyncssh.SFTPClient):
                            logger.info(f"Using AsyncSSH open+write method instead of get")
                            async with await self._sftp_client.open(remote_path, 'rb') as remote_file:
                                content = await remote_file.read()
    ...
    async def read_file(self, remote_path: str, start_line: int = 0, max_lines: int = -1) -> Optional[List[str]]:
        ...
        """
        # Initialize class-level attribute for caching paths to reduce log spam
        # This is a workaround for the 'method' object has no attribute 'logged_paths' error
        # By storing the cache on the class rather than the method
        if not hasattr(SFTPClient, '_file_path_cache'):
            SFTPClient._file_path_cache = set()
        
        # Only log first-time accesses to this file path
        first_time = remote_path not in SFTPClient._file_path_cache
    ...
    async def find_files_by_pattern(self, directory: str, pattern: str, recursive: bool = True, max_depth: int = 10) -> List[str]:
        ...
        try:
            ...
            logger.error(f"Error in find_files_by_pattern: {e}f")
            return []
    ...
    async def find_files_recursive(self, directory: str, pattern: str, result: List[str], recursive: bool = True, max_depth: int = 10) -> None:
        ...
        try:
            ...
            logger.error(f"Error in find_files_recursive: {e}f")
    ...
    async def exists(self, path: str) -> bool:
        ...
        try:
            ...
            logger.debug(f"Path does not exist or error accessing {path}: {e}f")
            return False
    ...
    async def is_file(self, path: str) -> bool:
        ...
        try:
            ...
            logger.debug(f"Error checking if path is a file {path}: {e}f")
            return False
    ...
    async def _find_files_recursive(self, directory: str, pattern_re: re.Pattern, result: List[str], recursive: bool, max_depth: int, current_depth: int) -> List[str]:
        ...
        try:
            ...
            # All scanning should be at DEBUG level to avoid console spam
            ...
            for entry in entries:
                ...
                # STRICT FILE FILTERING: Only process entries that match our pattern
                # This is critical for avoiding .sav and other unwanted files
                # Skip any files that don't match our pattern immediately
                if pattern_re is not None and not pattern_re.search(entry):
                    # For CSV files, we ONLY want .csv files - nothing else
                    # Skip silently without logging to reduce console spam
                    continue
                
                # Additional check to exclude .sav files explicitly
                if entry.endswith('.sav'):
                    # Skip silently - no need to log every .sav file
                    continue
                ...
                try:
                    ...
                    # Check if it's a file matching our pattern (CSV or log)
                    if isinstance(entry_info, dict) and entry_info["is_file"] and pattern_re.search(entry):
                        # Only add to results without any logging (essential for reducing console spam)
                        result.append(entry_path)

                    # Recursively explore directories if needed
                    elif isinstance(entry_info, dict) and entry_info["is_dir"] and recursive:
                        # Only log at the top level to avoid spam
                        if current_depth <= 1:
                            logger.debug(f"Exploring subdirectory: {entry_path}")
                        # Pass result by reference for batch processing efficiency
                        await self._find_files_recursive(entry_path, pattern_re, result, recursive, max_depth, current_depth + 1)

                except Exception:
                    # Silently continue without logging every error
                    continue
    ...
    async def find_csv_files(
        ...
        CRITICAL OPTIMIZATION: This method now has strict file type filtering to
        only process .csv files and avoid excessive logging. It will ignore all other
        file types like .sav files, dramatically improving processing time and efficiency.

        This method finds ONLY CSV files in the specified directory, with support for:
        ...
        - Batch processing for improved resource usage
        - Only minimal logging to prevent console spam at scale
        ...
        try:
            ...
            if isinstance(e, (OSError, IOError, ConnectionError, asyncio.TimeoutError)):
                ...
                raise ConnectionError(f"SFTP operation failed: {e}f")
    ...
    async def get_latest_csv_file(self) -> Optional[str]:
        ...
        try:
            # Check if we have a mapping to a known server numeric ID
            from utils.server_identity import KNOWN_SERVERS
            
            server_uuid = self.server_id
            # Use original_server_id for path construction if available, otherwise fall back to server_id
            path_server_id = self.original_server_id if hasattr(self, 'original_server_id') and self.original_server_id else self.server_id
            
            # Check if this is a known server with a specific numeric ID
            if server_uuid in KNOWN_SERVERS:
                ...
                if numeric_match:
                    ...
                    logger.info(f"Extracted numeric portion ({extracted_id}) from UUID path_server_id")
